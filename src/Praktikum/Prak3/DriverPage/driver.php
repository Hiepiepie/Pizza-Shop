...
class Driver extends Page
{
    ...
    protected function getViewData()
    {
        ...
        $dataArray = array($orders,$orderDetails);
        return $dataArray;
    }
    ...
    protected function generateView() 
    {
        ...
        echo <<< EOT
        <h1>Lieferung Status</h1>
        <form action="driver.php" accept-charset="UTF-8" method="post">
        EOT;
        foreach ($orderDetails as $orderId => $orderDetail) {
            ...
            switch (count($stats)){
                case $finish:{
                    echo <<< EOT
                    <section>
                    <h3>Bestellungnummer {$orderId}</h3>
                    <p>Adresse: {$orderDetail[1]} <br>Total : {$formattedTotal} &euro;<br> Pizzas : 
                    EOT;
                    foreach ($names as $pizzas => $pizza)
                        echo "{$pizza}, ";
                    echo <<< EOT
                    <br>
                    <label for=$orderId.1><br>Finish</label>
                    <input type="radio" name='statuses[{$orderId}]' id=$orderId.1 value="2" checked/><br>
                    <label for=$orderId.2>In Delivery</label>
                    <input type="radio" name='statuses[{$orderId}]' id=$orderId.2 value="3" /><br>
                    <label for=$orderId.3>Delivered</label>
                    <input type="radio" name='statuses[{$orderId}]' id=$orderId.3 value="4" /><br> 
                    </p>
                    </section> 
                    EOT;
                    break;
                ...
                case $inDelivery:{
                    ...
                    <br>
                    <label for=$orderId.1><br>Finish</label>
                    <input type="radio" name='statuses[{$orderId}]' id=$orderId.1 value="2" /><br>
                    <label for=$orderId.2>In Delivery</label>
                    <input type="radio" name='statuses[{$orderId}]' id=$orderId.2 value="3" checked/><br>
                    <label for=$orderId.3>Delivered</label>
                    <input type="radio" name='statuses[{$orderId}]' id=$orderId.3 value="4" /><br>  
                    </section> 
                    EOT;
...
//                    echo "<p style='color: red'>NOT READY !</p>";
    ...
    protected function processReceivedData() 
    {
        ...
        if (isset($_POST['statuses'])){
            $statuses = $this->_database->real_escape_string($_POST['statuses']);
