...
class Baker extends Page
{
    ...
    protected function getViewData()
    {
       ...
       // error_reporting(E_ALL);
        ...
        $sqlQuery = "SELECT article.name, ordered_articles.* FROM ordered_articles JOIN article WHERE ordered_articles.f_article_id = article.id AND ordered_articles.status < 2";
    ...
    protected function generateView() 
    {
        ...
        if($this->ifDataReceived == true){
            echo "<h2 style='color: green'>Status aktualisiert !</h2>";
        }
        echo "<h1>Pizzas Status</h1>";
        echo <<< EOT
            <form action="baker.php" accept-charset="UTF-8" method="post">
            EOT;
        ...
        echo <<< EOT
            <br><input type="submit"  value="Aktualisieren" />
            </form>
            EOT;
    ...
    public function printArticleStatus($articleEachOrder): void
    {
        foreach($articleEachOrder as $orderId => $articleArray){
            echo "<section><h2>Bestellnummer {$orderId}</h2>";
            foreach ($articleArray as $article){
                $id = $article[0];
                $name = $article[1];
                $status = $article[2];
                echo "<p><b>{$name} Pizza </b> mit id : {$id} <br>";
                switch ($status){
                    case 0:{
                        echo <<< EOT
            <label for=$id.1>Bestellt</label>
            <input type="radio" name='statuses[{$id}]' id=$id.1 value="0" checked/><br>
            <label for=$id.2>Im Ofen</label>
            <input type="radio" name='statuses[{$id}]' id=$id.2 value="1" /><br>
            <label for=$id.3>Fertig</label>
            <input type="radio" name='statuses[{$id}]' id=$id.3 value="2" /><br>  
            </p>
            EOT;
                        break;
                    }
                    case 1:{
                        echo <<< EOT
            <label for=$id.1>Bestellt</label>
            <input type="radio" name='statuses[{$id}]' id=$id.1 value="0" /><br>
            <label for=$id.2>Im Ofen</label>
            <input type="radio" name='statuses[{$id}]' id=$id.2 value="1" checked/><br>
            <label for=$id.3>Fertig</label>
            <input type="radio" name='statuses[{$id}]' id=$id.3 value="2" /><br>  
            </p>
            EOT;
            ...
            <input type="radio" name='statuses[{$id}]' id=$id.2 value="1" /><br>
            <label for=$id.3>Fertig</label>
            <input type="radio" name='statuses[{$id}]' id=$id.3 value="2" checked /><br>  
            </p>
            EOT;
                        break;
                    }
