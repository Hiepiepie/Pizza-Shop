...
class Driver extends Page
{
    ...
    protected function generateView() 
    {
        ...
        foreach ($orderDetails as $orderId => $orderDetail) {
            $formattedTotal = htmlspecialchars(number_format($orderDetail["total"], 2));
            $stats = array();
            $names = array();
            ...
            switch (count($stats)){
                case $finish:{
                    ...
                    <p class="text-responsive">Adresse: {$orderDetail["address"]} <br>Total : {$formattedTotal} &euro;<br> Pizzas : 
                    EOT;
